...
class AppLogger {
  ...
  static const String _base64Placeholder = '[BASE64_OMITTED]';
  static const String _dataUrlPlaceholder = '[DATA_URL_OMITTED]';
  ...
  static Object? _sanitizeData(Object? value,
    ...
    if (value is String) {
      if (_isBase64DataUrl(value)) {
        return _dataUrlPlaceholder;
      }
      if (keyHint != null) {
        if (_isSensitiveKey(keyHint)) return '[REDACTED]';
        if (_isKnownBase64Field(keyHint, path)) {
          return _base64Placeholder;
        }
  ...
  static String _collapseBase64Text(String input,
    ...
    var output = input.replaceAllMapped(
        _base64DataUrlPattern, (_) => _dataUrlPlaceholder);

    if (_shouldCollapseRawBase64(output, keyHint: keyHint, path: path)) {
      output = _base64Placeholder;
    }
    return output;
  }
  ...
  static bool _isKnownBase64Field(String? keyHint, List<String> path) {
    ...
    final normalizedKey = _normalizePathSegment(lowerKey);
    if (normalizedKey.contains('base64') || normalizedKey == 'b64json') {
      return true;
    }
    if (normalizedKey != 'data') return false;
    final normalizedPath = path.map(_normalizePathSegment).toList();
    return normalizedPath.contains('inlinedata') ||
        normalizedPath.contains('inputaudio');
  }

  static bool _isBase64DataUrl(String value) {
    return _base64DataUrlPattern.hasMatch(value);
  }

  static String _normalizePathSegment(String segment) {
    return segment.toLowerCase().replaceAll(RegExp(r'[^a-z0-9]'), '');
  }
